...
const mhopen=$('.z-nav-list [data-tabwin-open]').eq(0);
const myhome={
    title:mhopen.text(),
    url:mhopen.attr('data-tabwin-open')
};
...
$(document).on('click','.tianjia-fangchan',function(){
    var data=$('.user-fangchan tbody tr:eq(0)').clone();
    data.find('option:eq(0)').prop('checked',true);
    data.find('input').val();
    data.find('a').removeClass('tianjia-fangchan').addClass('remove-fangchan').text('删除').attr('eui','danger');
    $('.user-fangchan tbody').append(data);
});

$(document).on('click','.remove-fangchan',function(){
