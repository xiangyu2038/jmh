            ...
            <div class='eui-calendar'>
                <div class='eui-calendar-main'>
                    <div class='eui-calendar-main-title'>
                        <a class='eui-prev-month eui-icon-zuo'></a>
                        ...
                        <a class='eui-next-month eui-icon-you'></a>
                    </div>
...
Eui.prototype.tabwin=dt=>{
    ...
    const load=(obj,st,ed)=>{
        ...
        if(not){
            ...
            cl();
            if(txt==z.only){
                z.l.append(`<a class="eui-tabwin-label ${z.cs} eui-tabwin-only" eui-tabwin-title="${txt}" eui-tabwin-url="${url}" href="javascript:;">${txt}`);
            }else{
                z.l.append(`<a class="eui-tabwin-label ${z.cs}" eui-tabwin-title="${txt}" eui-tabwin-url="${url}" href="javascript:;">${txt}<i class="eui-icon-close"></a>`);
            };
            z.w.append(`<div class="eui-tabwin-main ${z.cs}" eui-tabwin-title="${txt}" eui-tabwin-url="${url}"></div>`);
            if(z.start) z.start({name:txt,url:url});
            if(st) st({name:txt,url:url});
            $('.eui-tabwin-main:last').load(url,function(responseTxt,statusTxt,xhr){
    ...
    // 防止重复绑定
    $('[data-tabwin-open],[data-tabwin-update]').off('click');
    // 绑定新增tab和刷新
    ...
    $(document).on('click','[data-tabwin-update]',function(){
        update($(this).data('tabwin-update'));
    });
    // 默认打开页面
...
Eui.prototype.layer=dt=>{
    ...
    z.dom+=
    `<div class="eui-layer" id="${z.id}" style="${z.style}">
    ...
    that.find('.eui-layer-close').off().click(function(){
        $('.eui-shade,.eui-layer').removeClass('on');
        setTimeout(()=>{
            that.remove();
        },200);
    });
